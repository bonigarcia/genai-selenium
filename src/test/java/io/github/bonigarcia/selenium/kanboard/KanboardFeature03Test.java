...
class KanboardFeature03Test {
    ...
    void testAddNewTask() {
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.name("title"))).sendKeys("task 3");
        driver.findElement(By.xpath("//button[text()='Save']")).click();

        // Assertions
        WebElement task = wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.xpath("//div[contains(@class, 'task-board-title')]")));
        assertTrue(task.getText().contains("task 3"), "Task 'task 3' is not displayed");
